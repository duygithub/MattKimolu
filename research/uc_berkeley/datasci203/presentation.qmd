...
* Conducting experiments on young swimmers is really hard (first-hand experience).
...
We use @fig-exploratory_data_stroke_count_elapsed_time_plot to help inform our model. We use the summary statistics, excluding those without standard deviations to generate test data from $N(\bar{x}, s)$.
...
Compared to the previous model, we achieved a much larger $R^2$ (`r model_v2_summary_data$r_squared`), but incrementally larger adjusted $R^2$ (`r model_v2_summary_data$adjusted_r_squared`) and with estimators with higher $p$ values, indicating that this model suffers from overfitting.`
...
Compared to the previous models, we had a much lower $R^2$ (`r model_v3_summary_data$r_squared`), but the largest adjusted $R^2$ (`r model_v3_summary_data$adjusted_r_squared`) and statistical significance. However, this model doesn't make sense to us because the model is missing interaction terms, which should matter.
