...
model3 <- lm(elapsed_time ~ stroke_count + I(stroke_count^2) + I(stroke_count^3) , data =dat )


summary(model3)

# Likelihood-ratio test (via anova for lm)
anova(model2, model3)        # test whether the cubic term adds explanatory power
...
# predictions + 95% CI
p <- predict(model3, newdata = newdat, se.fit = TRUE)
newdat <- newdat %>%
  mutate(fit = p$fit, se = p$se.fit,
         lwr = fit - 1.96*se, upr = fit + 1.96*se)
...
par(mfrow = c(2,2))
plot(model3, which = 1)  # Residuals vs Fitted (linearity/variance check)
plot(model3, which = 2)  # Normal Q-Q (normality check)
plot(model3, which = 3)  # Scale-Location (homoscedasticity)
plot(model3, which = 4)  # Residuals vs Leverage (influence)
