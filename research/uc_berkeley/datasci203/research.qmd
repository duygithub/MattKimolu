...
data_plot_custom_shapes <- c("25" = 3, "50L1" = 4, "50L2" = 15, "100IM" = 17)
...
data_files_names <- c("20251118_-8.csv", "20251121_-8.csv", "20251122_-8.csv", "20251129_-8.csv")
main_data <- compile_data_files(data_files_names)
prepped_main_data <- prep_data(main_data)

summary_main_data_stats <- summarize(
  group_by(prepped_main_data, stroke_count),
...
prepped_main_data2 <- prepped_main_data %>%
...
model1_coef_tbl <- as.data.frame(summary(model1)$coefficients)
model1_coef_tbl <- rownames_to_column(model1_coef_tbl, var = "Coefficient")

model1_coef_tbl2 <- model1_coef_tbl %>%
...
model2_coef_tbl <- as.data.frame(summary(model2)$coefficients)
model2_coef_tbl <- rownames_to_column(model2_coef_tbl, var = "Coefficient")

model2_coef_tbl2 <- model2_coef_tbl %>%
  select("Coefficient", "Estimate" ,     "Pr(>|t|)"  ) %>%
  rename(Value = `Estimate`,
         Pvalue = `Pr(>|t|)`)
...
kable(model1_coef_tbl2, digits = 3, caption = "Model 1 Summary")
# kable(model2_coef_tbl2, digits = 3, caption = "Model 2 Summary")
...
``` {r descriptive analysis by data entry type, echo=FALSE, message=FALSE, warning=FALSE}

#| echo: false
#| message: false
#| warning: false

summary_data_entry_type_25_50L1 <-prepped_main_data2 %>%
  filter(data_entry_type == "25 or 50L1") %>%
    group_by( stroke_count) %>%
    summarize(n=length((event_ordinal)),
              Mean = mean(time_milliseconds, na.rm = TRUE),
              SD=sd(time_milliseconds, na.rm = TRUE),
              SE = sd(time_milliseconds, na.rm = TRUE)/sqrt(length(time_milliseconds)),
              Min=quantile(time_milliseconds,probs = 0,type=2,na.rm = TRUE) ,
              Q1=quantile(time_milliseconds,probs = 0.25,type=2,na.rm = TRUE),
              Median= median(time_milliseconds, na.rm = TRUE),
              Q3=quantile(time_milliseconds,probs = 0.75,type=2,na.rm = TRUE),
              Max=quantile(time_milliseconds,probs = 1,type=2,na.rm = TRUE)
    ) %>%
  rename(`Stroke Count` = stroke_count)
  

summary_data_entry_type_50L2 <-prepped_main_data2 %>%
...
summary_data_entry_type_100IM <-prepped_main_data2 %>%
...
## Model Included unwell_feedback and event_ordinal
...
\end{split}
$$ {#eq-regression_model_v}

The next iteration of our test incorporated the unwell_feedback and event_ordinal. Compared to @eq-regression_model_v1, we achieved a smaller residual, but with variables that have much higher $p$ values, indicating that this model suffers from overfitting.
