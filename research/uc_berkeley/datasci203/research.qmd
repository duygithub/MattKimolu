...
: Rest Times between Timed Tests {#tbl-rest_times}
:::

## Warm-Ups
Ahead of each session, the swimmer should swim the following 900-yard warm-up routine:

1. 2x 100 SCY. BR, FR. Pace: Easy, Build. Rest 30 seconds after FR.
1. 2x 50 SCY. BR kick on back, FR kick on stomach. Pace: Build, Push. Rest 30 seconds after BR, rest 1 minute after FR.
1. 4x 50 SCY. Drill on odd laps and normal swim on the even laps. Pace: Build. Rest 15 seconds after every set.
1. 4x 50 SCY. Broken 200 IM SCY. Pace: Sprint. Rest 30 seconds after every set.
...
> **$H_0$**: Given an $S_{C_{optimal}}$ ~ $T_{lap_{optimal}}$, there is no difference of at least .5 second between $T_{lap_{optimal}}$ and $T_{lap_{-2}}$ and $T_{lap_{optimal}}$ and $T_{lap_{+2}}$ where $T_{lap_{-2}}$ ~ $S_{C_{optimal}} - 2$ strokes and $T_{lap_{+2}}$ ~ $S_{C_{optimal}} + 2$ strokes.
...
We choose .5 second for our 25-yard lap because the difference between a top and second -tier national swimmer[^6] for a 50 BR SCY is $\le$ 2 seconds[^7]. A .5 second for each lap would equal a 1 second improvement for 50 SCY and the swimmer can work on glide time and turns to improve the time by another second.
...
$$
T_{lap} = \beta_0 + \beta_1 S_C + \beta_2 {S_C}^2 + \gamma_{50L2} I_{50L2} + \gamma_{100IM} I_{100IM} + \delta_{50L2}S_CI_{50L2} + \delta_{100IM}S_CI_{100IM} + \epsilon
$$ {#eq-linear_regression_model}
...
| $\delta_{50L2}$, $\delta_{100IM}$ | $\delta$'s operationalizes the fatigue from previous laps, thereby changing the slope of the model based on the change in the cost or benefit of each additional stroke.  |
...
: Definition of Linear Model Components {#tbl-definition_of_linear_model_components}
:::
...
summary_stats <- summarize(group_by(prepped_data, stroke_count), mean_time = as.numeric(mean(time_milliseconds)), standard_deviation=sd(time_milliseconds))
...
#| message: false
#| warning: false
ggplot(exploratory_data, aes(x = factor(prepped_data$stroke_count), y = prepped_data$time_milliseconds)) +
...
We use @fig-exploratory_data_stroke_count_elapsed_time_plot to help inform us of how to model @eq-linear_regression_model. We use the summary statistics, excluding those without standard deviations, in @tbl-summary_statistics_exploratory_data to generate test data from $N(\bar{x}, s)$. This allows us to execute our power analysis.

```{r exploratory_data_power_analysis, echo=FALSE, message=FALSE, warning=FALSE}

# stats_with_sd <- summary_stats[!is.na(summary_stats$standard_deviation), ]
...
make_data_from_exploratory_data <- function(summary_df, total_n) { 
  ...
  # Get pooled SD (assume it's the same for all rows, use the first)
  sd_pooled <- summary_df$pooled_sd[1]
...
exploratory_data_simulated_data<- make_data_from_exploratory_data(summary_stats2, 1000) 


power_analysis <- function(simulation_count, exploratory_data_simulated_data) {
  reject_vector <- NA
  
  for (i in 1 : simulation_count) {
    # exploratory_data_simulated_data <- make_data_from_exploratory_data(data_size, input_data)
...
# power_analysis_data_size <- 17

exploratory_data_power <- power_analysis(simulation_count=power_analysis_simulation_count, exploratory_data_simulated_data)
```
...
$$
\hat{T}_{lap} = \hat{\beta}_0 + \hat{\beta}_1 S_C + \hat{\beta}_2 {S_C}^2 + \hat{\gamma}_{50L2} I_{50L2} + \hat{\gamma}_{100IM} I_{100IM} + \hat{\delta}_{50L2}S_CI_{50L2} + \hat{\delta}_{100IM}S_CI_{100IM}
$$ {#eq-fitted_linear_regression_model}
...
<!-- | **Variable** | **Reason** | -->
<!-- | :--- | :--- | -->
<!-- | Age | Competitive swimming requires significant concentration to improve technique. Age is a proxy for maturity. We can use this variable to weight older data. We can also This variable could also be useful for selecting the optimal $S_C$ where associated with a high $s$ but fast $\bar{x}$ or $x_{fast}$. | -->
