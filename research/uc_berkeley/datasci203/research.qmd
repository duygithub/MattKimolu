...
```{r, Model Validation beta scatter plot}
...
```
By using Model v1, we have calculated standardized residual, leverage ($h_{ii}$), and Cook's distance ($D_i$), to identify data points those are outlines, leverage points and influential points. 

There are 2 data points those are both outlines and influential points. 

We compared the full data and subset data fits by examining (i) overall model significance, (ii) coefficient estimates and their standard errors, and (iii) the estimated effect of stroke count (including the quadratic term) and its interaction with data entry type. (Pending)
