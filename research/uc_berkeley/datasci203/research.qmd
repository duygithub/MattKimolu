...
Given the swimmer is a minor engaged in a strenuous physical activity, there are safety considerations. Therefore, additional rest times beyond those defined in @tbl-rest_times will be granted upon the swimmer's request and this information will be reflected in the data.
...
> **$H_0$**: $\left| T_{Lap_{Optimal}} - T_{Lap_{-2}} \right| \le 500$ and $\left| T_{Lap_{Optimal}} - T_{Lap_{+2}} \right| \le 500$ where $T_{Lap_{Optimal}} \sim S_{C_{Optimal}}$, $T_{Lap_{-2}} \sim S_{C_{Optimal}} - 2$ strokes, and $T_{Lap_{+2}} \sim S_{C_{Optimal}} + 2$ strokes.
...
ggplot(exploratory_data, aes(
  x = factor(prepped_exploratory_data$stroke_count),
  # y = prepped_exploratory_data$time_milliseconds)) +
  y = as.numeric(prepped_exploratory_data$time_milliseconds))) +
geom_point(width = 0.2, alpha = 0.7) + # `alpha` adds transparency
scale_y_continuous(
...
Jadzia Vo swam Y laps over X sessions. Each session happened on a different day.

```{r compile_main_data, echo=FALSE, message=FALSE, warning=FALSE}
main_data_file1 <- read.csv("data/20251112_-8.csv")
main_data_file2 <- read.csv("data/20251120_-8.csv")

main_data <- rbind(main_data_file1, main_data_file2)
...
summary_main_data_stats <- summarize(group_by(prepped_main_data, stroke_count), mean_time = as.numeric(mean(time_milliseconds)), standard_deviation=sd(time_milliseconds))
```

```{r plot_main_data}
#| echo: false
#| fig-cap: "Distribution of Elapsed Time (in milliseconds) for each Stroke Count by data_entry_type"
#| fig-height: 6
#| fig-pos: H
#| fig-cap-location: top
...
main_data_y_max <- 32000
main_data_y_seq <- seq(from = main_data_y_min, to = main_data_y_max, by = 500)

main_data_plot_custom_shapes <- c("25" = 3, "50L1" = 4, "50L2" = 15, "100IM" = 17)
main_data_plot_custom_colors <- c("25" = "blue", "50L1" = "blue", "50L2" = "green", "100IM" = "red")


ggplot(main_data, aes(x = factor(prepped_main_data$stroke_count),
                      y = as.numeric(prepped_main_data$time_milliseconds),
                      color = prepped_main_data$data_entry_type, 
                      shape = prepped_main_data$data_entry_type)) +
geom_point(width = 0.2, alpha = 0.7, size = 3, stroke = 2) + # Increased size slightly for visibility
scale_color_manual(values = main_data_plot_custom_colors) +
scale_shape_manual(values = main_data_plot_custom_shapes) +
scale_y_continuous(
...
labs(
  ...
  x = expression(bold(S[C])),
  y = expression(bold(T[lap])),
  color = "data_entry_type", # Legend Title
  shape = "data_entry_type"  # Legend Title (must match color title to combine legends)
) +
...
| **$S_C$** | **$\bar{x}$** | **$x_{Fastest}$** | **$s$** | **$x_{Slowest}$** | **$\tilde{x}$** |
| :--- | :--- | :--- | :--- | :--- | :--- |
...
## Use Varying Coefficients for 50, 100, and 200 BR's
