...
# 
# 
# ## Model v1, without transformed stroke count
# 
# 
# 
# 
...
# BC <- boxcox( model_v3, lambda = seq(0, 10, 1/100), plotit = FALSE)
# BC_df <- data.frame(
#   logLik = BC$y,
#   lambda = BC$x
# )
...
# summary( model_v4 )
# 
# stargazer(model_v1, model_v4,type = "text" )


```

We would like to improve our Model v1, so we tried the Boxcox transformation. The result of the Boxcox transformation indicated that the when lambda = 4.02, the likelihood reached max. It indicates that the Boxcox transformation suggests that to transfer the response variable with lambda of 4.02. So our transformation y is the following:

$$
y^{(\lambda)} = \dfrac{y^{\lambda} - 1}{\lambda} \text{, where lambda = 4.02}
$$
Need to add model comparison later.
