...
| $\eta U$ | $U$ operationalizes if the swimmer is feeling undwell and $\eta$ is the corresponding regression coefficient. |
...
| unwell_feedback | Operationalizes if the swimmer is not feeling well, usually because they are tired or are not in the mood to swim. Captured as $U$ in the model. Defaults to 0. 1 means they are unwell and -1 means they are feeling great. |
...
geom_text(aes(label = prepped_exploratory_data$event_ordinal), 
          nudge_x = 0.1,      # Move slightly to the right
          nudge_y = -100,     # Move slightly down (Adjust this value based on your Y scale!)
...
data_files_names <- c("20251118_-8.csv")
...
geom_text(aes(label = prepped_main_data$event_ordinal), 
          nudge_x = 0.1,      # Move slightly to the right
          nudge_y = -220,     # Move slightly down (Adjust this value based on your Y scale!)
          size = 3,           # Make font smaller than the dots
          color = "black",    # Keep text black so it's readable
          fontface = "bold",
          show.legend = FALSE # Don't add 'a' to the legend
