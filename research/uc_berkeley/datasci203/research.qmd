...
* Encourages swimmers to listen to their bodies and think about how they feel when they move through the water.
...
$S_C$ is the total number of arm strokes a swimmer takes to complete one length of the pool. $S_C$ is proportional to $S_R$ and inversely proportional to $S_L$. Accounting for the glide distance (how far the swimmer is able to dive and swim underwater or kick off the wall and swim underwater),
...
After attempting several models outlined in @sec-appendix_a, we settle on our final regression model:

$$
\begin{split}
T_{Lap} = & \beta_0 + \beta_1 S_C + \beta_2 {S_C}^2 \\
& + \gamma_{50L2} I_{50L2} \\
& + \gamma_{100IM} I_{100IM} \\
& + \eta_0 O + \eta_1 O S_C + \eta_2 O {S_C}^2 \\
& + \epsilon
\end{split}
$$ {#eq-regression_model_final}

::: {tbl-colwidths="[20,80]"}
| Component(s) | Definition |
| :--- | :--- |
| $T_{Lap}$ | The timing of how long it took for the swimmer to complete the breaststroke swim. For the data entry of types 25 and 50L1, this is the time it takes the swimmer to swim the first lap. For 50L2, this is the time it takes the swimmer to swim the second lap. For 100IM, this is the time it takes for the swimmer to swim the breaststroke leg of the IM. The values are in milliseconds.
| $\beta_0$ | This is the time needed to travel the entire distance of the first lap, assuming no stroke is taken. This is theoretically achievable for any stroke in a 25 SCY, 25 SCM, and 50 LCM scenario by a swimmer diving in and gliding to the other side without taking a single stroke.|
...
| $\eta_0 O$, $\eta_1 O S_C$, $\eta_2 O S_C^2$ | $O$ is the event_ordinal from the data file and $\eta$'s are the corresponding regression coefficient. Operationalizes the tradeoff between fatigue over numerous trials vs flow state. |
...
We use @fig-exploratory_data_stroke_count_elapsed_time_plot to help inform us of how to model @eq-regression_model_final. We use the summary statistics, excluding those without standard deviations, in @tbl-summary_statistics_exploratory_data to generate test data from $N(\bar{x}, s)$.

```{r exploratory_data_power_analysis, echo=FALSE, message=FALSE, warning=FALSE}
source("source/descriptive_summary.R")
source("source/exploratory_data_power_analysis.R")


exploratory_summary <- descriptive_summary(prepped_exploratory_data)

power_analysis_simulation_count <- 1000
power_analysis_data_size <- 60
alpha <- 0.05
...
We ran `r power_analysis_simulation_count` simulations and determined that our swimmer should swim at least `r power_analysis_data_size` laps to achieve a power of approximately `r exploratory_data_power` for $\alpha$ = `r alpha`.
...
In addition to the data collected for the power analysis, Jadzia Vo swam an additional `r number_of_br_laps` 25 SCY BR laps over `r number_of_br_sessions` sessions. Each session happened on a different day.
...
<!-- After attempting several models outlined in @sec-appendix_a, we settle on our final model: -->
...
<!-- & + \hat{\eta}_0 O + \hat{\eta}_1 O S_C + \hat{\eta}_2 O {S_C}^2 \\ -->
<!-- & + \epsilon -->
<!-- \end{split} -->
...
<!-- * 50L2 -->
<!-- * 100IM -->
<!-- (we don't attempt to find the optimal $S_C$ for this value because we have a data set that is far too small for analysis.) -->
...
<!-- The quadratic formula for $S_C$ gives us additional options when $T_{Target}$ is not the vertex because there are two $S_C$'s in the solution (from too few strokes and too many strokes). Given 2 $S_C$'s, with all else being equal, we would likely choose the one with lower variance for a 25 BR SCY. -->
...
$$
\begin{split}
\hat{T}_{Lap} = & \hat{\beta}_0 \\
 & + \hat{\gamma}_{50L2} I_{50L2} \\
 & + \hat{\gamma}_{100IM} I_{100IM} \\
 & + \hat{\eta}_0 O
\end{split}
$$ {#eq-regression_model_v0}

This is the baseline model that we use to compare our model against. It does not include any $S_C$.

```{r, hypothesis testing results, echo=FALSE, message=FALSE, warning=FALSE}
...
model_v0_summary_data <- calculate_model_coefficients_summary(model_v0)

knitr::kable(model_v0_summary_data$table_summary_data, digits = 3, caption = "Model v0 Summary")

model_final <- lm(time_milliseconds ~ 
                stroke_count + 
                I(stroke_count^2) + 
                data_entry_type + 
                event_ordinal + 
...
test1 <- anova(model_v0, model_final)

hypothesis_p_value <- unique(test1$`Pr(>F)`)
hypothesis_p_value <- round(na.omit(hypothesis_p_value)[1],4)

hypothesis_p_value2 <- ifelse(hypothesis_p_value< 0.0001, 
                              "<0.0001", hypothesis_p_value)


```

### Result 1: Stroke count significantly affected swimming time

To test if stroke count significantly affects swimming time, we compared our model defined by @eq-regression_model_final with the baseline model defined by @eq-regression_model_v0 with the ANOVA test.

The result of the F-test from ANOVA gave us a p-value of `r hypothesis_p_value2` which is significant, which indicates that stroke count is meaningful for predicting swim time for swimmers under 11-years-old.

### Result 2: We discovered a flaw in our data collection

The following table identifies optimal stroke count for each data_entry_type and event_ordinal. Notice that after event_ordinal 6, the optimal $S_C$ no longer seems to change. This is likely evidence that the swimmer is now too fatigued to produce IID data. In a normal competition day, over a period of 3+ hours, swimmers do not compete in more than 4 events (the maximum is usually 3 events in 3 hours). This points to the fact that our rest period is either not enough, or is only valid up until 6 trials. When we started this data collection, the event_ordinal value was only collected in case we ever needed to review the data and was never meant to be part of our model. The fact event_ordinal is a significant part of our model points to a data collection failure.

```{r optimal Stroke count, echo=FALSE, message=FALSE, warning=FALSE}

## Simulate testing data:

stroke_seq  <- with(main_data_for_modeling, seq.int(min(stroke_count, na.rm = TRUE),
                                                   max(stroke_count, na.rm = TRUE),
                                                   by = 1))

ordinal_seq <- with(main_data_for_modeling, seq.int(min(event_ordinal, na.rm = TRUE),
                                                   max(event_ordinal, na.rm = TRUE),
                                                   by = 1))

type_seq <- unique(main_data_for_modeling$data_entry_type)

pred_data <- expand.grid(
  event_ordinal    = ordinal_seq,
  stroke_count     = stroke_seq,
  data_entry_type  = type_seq,
  KEEP.OUT.ATTRS   = FALSE,
  stringsAsFactors = FALSE
)

# Get predictions with 95% confidence intervals
pred_results <- predict(model_final , newdata = pred_data, interval = "confidence", level = 0.95)
pred_data2 <- cbind(pred_data, pred_results)
 

predicted_summary <- pred_data2  %>%
    group_by(data_entry_type, event_ordinal ) %>%
    summarize(
              Min=quantile(fit,probs = 0,type=2,na.rm = TRUE)
    )

pred_data3 <- pred_data2 %>%
  left_join(predicted_summary, by = c("data_entry_type", "event_ordinal")) %>%
  mutate(is_min = ifelse(fit == Min, "Yes", "No")) %>%
  filter(is_min == "Yes") %>%
  dplyr::select(data_entry_type, event_ordinal, stroke_count,
         Min) %>%
  mutate(Min = round(Min,0)) %>%
  rename(`Data Entry Type` = data_entry_type,
         `Event Ordinal` = event_ordinal,
         `Optimal stroke Count` = stroke_count,
         `Best Swim Time` = Min)


knitr::kable(pred_data3, digits = 3, caption = "Optimal Stroke count per data type and ordinal events")

```


<!-- ```{r optimal stroke count plot, echo=FALSE, message=FALSE, warning=FALSE  , fig.width=10, fig.height=7} -->
...
<!-- #| fig-pos: H -->
<!-- #| fig-cap-location: top -->
<!-- #| label: plot_optimal_stroke_count -->
<!-- #| message: false -->
<!-- #| warning: false -->
...
<!-- p_plot_with_model <- ggplot(main_data_for_modeling, -->
<!--        aes(x = as.numeric(stroke_count), -->
<!--            y = as.numeric(time_milliseconds), -->
<!--            color = factor(data_entry_type))) + -->
...
<!-- ) + -->
<!-- labs( -->
<!--   title = "model_final: Optimal Stroke Count (Average Ordinal Event = 6)", -->
<!--   x = expression(bold(S[C])), -->
<!--   y = expression(bold(T[lap])), -->
<!--   # color = "unwell_feedback", # Legend Title -->
<!--   # shape = "data_entry_type", # Legend Title -->
...
<!-- Once we've defined $T_{Target}$ for breaststroke, we can use @eq-stroke_count_for_target_time_100IM to find $S_C$ for breaststroke that is a good minimizer for the 100 IM. -->
...
Looking at @fig-residual_vs_fitted_model_final, Jadzia seems to have a consistent-enough kicking technique to generate homoskedastic data.

```{r qq_plot_main_model}
#| echo: false
#| fig-cap: "Residuals vs Fitted Values Plot of Linear Model"
#| fig-pos: H
#| label: fig-residual_vs_fitted_model_final
...
# Appendix A - Evolution of Regression Model {#sec-appendix_a}
...
Compared to @eq-regression_model_v1, we achieved a much larger $R^2$ (`r model_v2_summary_data$r_squared`), but incrementally larger adjusted $R^2$ (`r model_v2_summary_data$adjusted_r_squared`) and with estimators with higher $p$ values, indicating that this model suffers from overfitting. Moreover, we didn't achive any significance with the coefficients.

## Model V3: Simplify to get to Stastical Significance
...
\hat{T}_{Lap} = & \hat{\beta}_0 + \hat{\beta}_1 S_C + \hat{\beta}_2 {S_C}^2 \\
 ...
 & + \hat{\gamma}_{100IM} I_{100IM}
\end{split}
$$ {#eq-regression_model_v3}

We simplified the model 3 to get to see if we can get to statistical significance.
...
This model produced $R^2$ (`r model_v3_summary_data$r_squared`) that is greater than  @eq-regression_model_v1 but less than @eq-regression_model_v2, but the lowest adjusted $R^2$ (`r model_v3_summary_data$adjusted_r_squared`). Moreover, this model doesn't make sense to us because the model is missing interaction terms, which should matter.
...
This leads us to our final comparison among our models. Our model, @eq-regression_model_final, has the highest adjusted $R^2$, the most number of significant terms, and the lowest robust standard error.

```{r compare models, echo=FALSE, message=FALSE, warning=FALSE}

source("source/modelSummary2df.R")

beta_rename <- read.csv("source/beta_rename.csv")


model_summary_list<- rlang::set_names(
  list(model_v1, model_v2, model_v3, model_final),
  c("model_v1", "model_v2", "model_v3", "model_final")
)

model_compare_df_1 <- modelSummary2df(model_summary_list)

model_compare_df_2 <- beta_rename %>%
  left_join(model_compare_df_1, by = c("old_name" = "term")) %>%
  dplyr::select(-old_name ) %>%
  rename(term = new_name)

    ## Sorting
    keep_last <- c("[Sample Size]",
                   "[R^2]",
                   "[Adjusted R^2]",
                   "[RSE]",
                   "[Test Statistic]",
    ...
    model_compare_df_2 <- model_compare_df_2 %>%
      arrange(
        term %in% keep_last,
...
knitr::kable(model_compare_df_2 , digits = 3, caption = "Model Comparison")
...
# Appendix B - Model Adequacy Checking


## Linear Relationship Between Variables

To examine relationships among variables, we generated a scatter plot matrix with pairwise correlations. 

About relationship between response variable vs predictor variables, looking at the first row and column, we observe how time_milliseconds relates to each predictor. Correlations with event_ordinal $(r = 0.03)$ and stroke_count $(r = 0.07)$ are very weak, while plots show no obvious nonlinear patterns either. The variable data_entry_type end up with a higher correlation coefficient $(r = 0.82)$, but notice that this variable is a categorical variable, so the Pearson correlation is not strictly appropriate in this case. 

About relationships within predictor variables, correlation coefficients among predictors are all low $(|r| ≤ 0.17)$, so the risk of multicollinearity is minimal. 

```{r relationship bettween variables plot,  echo=FALSE, message=FALSE, warning=FALSE , fig.width=10, fig.height=8}

#| echo: false
#| fig-cap: "relationship bettween variables plot"
##| fig-width: 12
##| fig-height: 10
#| fig-pos: H
#| fig-cap-location: top
...
main_data_for_modeling3 <- main_data_for_modeling2 %>%
  dplyr::select(time_milliseconds, 
                event_ordinal, 
                stroke_count,
                data_entry_type)

panel.cor <- function(x, y, digits = 2, method = "pearson", cex.cor = 1.2, ...) {
  
  if (!is.numeric(x)) x <- as.numeric(x)
  if (!is.numeric(y)) y <- as.numeric(y)

  r <- suppressWarnings(cor(x, y, use = "pairwise.complete.obs", method = method))
...
```{r Constant Variance bptest, echo=FALSE, message=FALSE, warning=FALSE}

model <- model_final

bptest_result <- round(bptest(model)$"p.value",4)

```


We have used 3 different approaches to examine constant variance, and they support the constant variance assumption.

Those Residual plots show residuals scattered in a band of approximately uniform thickness across fitted values, with no funnel-shaped pattern.

The Scale-Location plot confirms this, as the smoothed line remains relatively flat.

The Breusch-Pagan test reported a p-value of `r bptest_result``, which failed to reject null hypothesis, so there is no evidence of heteroscedasticity.


```{r Constant Variance Plot,  echo=FALSE, message=FALSE, warning=FALSE , fig.width=10, fig.height=9}

#| echo: false
#| fig-cap: "Constant Variance Plot"
##| fig-width: 12
##| fig-height: 10
#| fig-pos: H
#| fig-cap-location: top
#| label: plot_constant_variance
#| message: false
#| warning: false
#| 
...
```{r Shapiro-Wilk Normality Test,  echo=FALSE, message=FALSE, warning=FALSE }
...
shapiro_test_pvalue <- shapiro.test(model$residuals)$"p.value" 
shapiro_test_pvalue2 <- ifelse(shapiro_test_pvalue < 0.0001, "<0.0001",
                              round(shapiro_test_pvalue,4 ))
shapiro_test_W <- round(shapiro.test(model$residuals)$"statistic",4)

```

We examine normality of residuals using histogram, Q-Q plot, and the Shapiro-Wilk test. Combined of those 3 results we concluded that residuals violate the normality assumption. This violation seems driven by outliers, we will later have analysis for outliers to further study about them. 

The histogram reveals a roughly symmetric distribution centered near zero, but with an extreme negative outlier creating left skewness. The Q-Q plot shows that while the central portion of residuals follows the theoretical line, but there is a severe departure in the lower left tail driven by observations 41 and 47 and other outliers, which shows heavier tails than normal distribution. The result of Shapiro-Wilk test (W = `r shapiro_test_W`, `r shapiro_test_pvalue2`) indicates that the null hypothesis of normality has been rejected. 


```{r Residual qq plot,  echo=FALSE, message=FALSE, warning=FALSE , fig.width=10, fig.height=7}

#| echo: false
#| fig-cap: "Residual qq plot"
##| fig-width: 12
##| fig-height: 10
#| fig-pos: H
#| fig-cap-location: top
#| label: plot_residual_qqplot
#| message: false
#| warning: false
...
We want to make sure that @eq-regression_model_final not only fits the current dataset well, but also remains valid for future or unseen data. To do this, we tested the stability of the coefficients by repeatedly splitting the data and re-estimating the model. For the data splits, we randomized the samples while stratifying by “data_entry_type” to ensure that each set had balanced characteristics. After randomization, each sample was assigned to either the estimation set (for fitting the model) or the validation set (for evaluating prediction error). We refit the model using the estimation set and then calculated the mean squared prediction error using the validation set. This process was repeated 1,000 times.

From the beta distribution plots, we can conclude the following: (a) Regarding stability, most beta histograms are symmetric and narrow. This suggests that the coefficients are stable, since their estimates are mostly normally distributed. However, Beta 7 and Beta 8 appear slightly skewed, which may be because they are interaction terms. (b) Regarding sign consistency, the medians of Beta 1 and Beta 2 are close to 0, which suggests that we cannot reliably interpret the direction of their effects. The remaining beta coefficients have consistent signs.

The MSP distribution plot is strongly right skewed. This indicates that most MSP values are small, meaning the model’s predictions are generally reasonable. However, there are a few cases with large MSP values, which may be due to outliers in the validation set.



```{r, Model Validation, echo=FALSE, message=FALSE, warning=FALSE, fig.width=10, fig.height=6}
#| echo: false
#| fig-cap: "Distribution of Beta"
##| fig-width: 10
##| fig-height: 5.5
#| fig-pos: H
#| fig-cap-location: top
#| label: plot_beta_distribution
#| message: false
#| warning: false
...
p1 <- make_hist(beta1, "beta1 distribution: \nStroke Count")
p2 <- make_hist(beta2, "beta2 distribution: \nStroke Count^2")
p3 <- make_hist(beta6, "beta6 distribution: \nEvent Ordinal")
p4 <- make_hist(beta7, "beta7 distribution: \nStroke Count x Event Ordinal")
p5 <- make_hist(beta8, "beta8 distribution: \nStroke Count^2 x Event Ordinal")
p6 <- make_hist(MSP, "MSP distribution: \nMean Squared Prediction Error")
...
## Model without Outliers, Leverage Points and Influential points

To check if there are any observations that affected the @eq-regression_model_final, we examined outliers, leverage points, and influence points. 

We have applied the following criterias:

Outliers: Standardized residuals $> 2$

Leverage points: $H_{ii} > 2 \cdot p/n = 2 * 8 / 66 = 0.24$

Influential points: Cook's distance $> 4 / n = 4 / 66 = 0.06$

There are 5 observations those are outliers, leverage points, or influence points. We have attached detailed info about those observations and also indicated them on the graph.


```{r, Outlier,Leverage,Influential, echo=FALSE, message=FALSE, warning=FALSE}
...
# write.csv(main_data_for_modeling2, "data/combined_data.csv", row.names = FALSE)
...
cooks_d <- data.frame(d)
cooks_d$rowid <- as.integer(rownames(cooks_d )) 
...
dfbetas_result <- data.frame(dfbetas(model))
dfbetas_result$rowid <- 1:nrow(dfbetas_result)

dffits_result <- data.frame(dffits(model))
...
n <- nrow(dat3)  # sample size
p <- length(coef(model))  # number of parameters

# Determine: INFLUENTIAL POINTS (based on DFFITS(beta))
dffits_cutoff <- 2 * sqrt(p/n)
dataset99$influential_dffits <- abs(dataset99$dffits.model.) > dffits_cutoff
...
# Determine OUTLIERS (Based on Standardized Residuals)
outlier_cutoff <- 2
dataset99$outlier <- dataset99$standard_res1 > outlier_cutoff


# Determine: INFLUENTIAL POINTS (Based on Cook's Distance )
cooks_cutoff <- 4 / n
...
dataset99_leverage <- dataset99 %>%
  filter(high_leverage == TRUE) %>%
  mutate(outlier_type = "Leverage Point")
...
dataset105 <- dataset102 %>%
  filter(outlier_type != "Non-Outlier,Leverage,Influential") %>%
  distinct(record_id, .keep_all = TRUE) %>%
  mutate(H_ii = round(H_ii,2 ),
         standard_res1 = round(standard_res1,2 ),
         Cooks_d = round(Cooks_d,2 )) %>%
  mutate(outlier = ifelse(outlier == "FALSE", "", outlier),
         high_leverage = ifelse(high_leverage == "FALSE", "", high_leverage),
         influential_cooks = ifelse(influential_cooks == "FALSE", "", influential_cooks),
         ) %>%
  # dplyr::select("record_id", "time_milliseconds",
  #               "outlier" , "high_leverage",  "influential_cooks",
  #               "standard_res1","H_ii" ,  "Cooks_d" , 
  #        "event_ordinal", "stroke_count" ,
  #        "data_entry_type"  , "time_milliseconds"  ) %>%
  dplyr::select("record_id", 
                "outlier" , "high_leverage",  "influential_cooks",
                "standard_res1","H_ii" ,  "Cooks_d"  ) %>%
  rename(`obs` = record_id,
         # `Time` = time_milliseconds,
         `Leverage Point` = high_leverage,
         `Outlier` = outlier,
         `Influential Point` = influential_cooks,
         `Standardized Residual` = standard_res1,
         ) 


knitr::kable(dataset105 , digits = 3, 
             caption = "Outlier, Leverage Points, and Influential Points")
...
```{r, Outlier,Leverage,Influential plot, echo=FALSE, message=FALSE, warning=FALSE, fig.width=12, fig.height=6}

#| echo: false
#| fig-cap: "Model without Outliers, Leverage Points and Influential points"
##| fig-width: 12
##| fig-height: 4
#| fig-pos: H
#| fig-cap-location: top
#| label: plot_model_final_no_outliers
#| message: false
#| warning: false
...
p_outlier <- ggplot() +
  ...
  labs(x = "Stroke Count",
       ...
       shape = "Outlier Type") +
  theme_minimal()+
  scale_x_continuous(breaks = function(x) seq(floor(min(x)-1), ceiling(max(x)+1), by = 1))+
...
We build a new model using the dataset without outliers, leverage points and influential points, and compared with the previous model. The new model_v6 has 61 samples has several improvements: (a) R^2 increased from 0.7579 to 0.9053 and (b) RSE reduced from 1656 to 981. However, stroke count is still not significant in the new model. 

```{r remove outlier model comparison , echo=FALSE, message=FALSE, warning=FALSE}
...
###########################
## Test model without Outlier,Leverage,Influential
...
model_summary_list<- rlang::set_names(
  list( model_final, model_v6),
  c("model_final", "model_v6")
)

model_compare_df_1 <- modelSummary2df(model_summary_list)

model_compare_df_2 <- beta_rename %>%
  right_join(model_compare_df_1, by = c("old_name" = "term")) %>%
  dplyr::select(-old_name ) %>%
  rename(term = new_name)
...
```{r boxcox transformation, echo=FALSE, message=FALSE, warning=FALSE}
...
## get lambda for boxcox transformation, and extract as df
## First try lambda from 0 to 10
## For y value transformation.
BC <- boxcox( model, lambda = seq(0, 10, 1/100), plotit = FALSE)
BC_df <- data.frame(
  logLik = BC$y,
  lambda = BC$x
)
...
## 95% CI threshold for lambda
BC_95ci <- max(BC$y) - 0.5*qchisq(0.95, 1)

## get max of lambda
BC_lambda_max <-  BC$x[which.max(BC$y)]
max_logLik <- max(BC$y)
...
We would like to improve our @eq-regression_model_final, we have tried the Boxcox transformation. The result of the Boxcox transformation indicated that the when lambda = `r BC_lambda_max`, the likelihood reached max. It indicates that the Boxcox transformation suggests that to transfer the response variable with lambda of `r BC_lambda_max`. So our $y$ after  transformation is the following:

$$
y^{(\lambda)} = \dfrac{y^{\lambda} - 1}{\lambda} 
$$


```{r boxcox plot,  echo=FALSE, message=FALSE, warning=FALSE, fig.width=10,              fig.height =2.6}

#| echo: false
#| fig-cap: "Boxcox Transformation"
##| fig-width: 12
##| fig-height: 10
#| fig-pos: H
#| fig-cap-location: top
...
#| 
## Ploting
bc_plot <- ggplot(BC_df, aes(x = lambda, y = logLik)) +
  geom_line(linewidth = 1) +
  geom_vline(xintercept = BC_lambda_max, linetype = "dashed", linewidth = 1, color = "red") +
  geom_hline(yintercept = BC_95ci, linetype = "dashed", linewidth = 1, color = "blue") +
  annotate("text",
           x = BC_lambda_max, y = max_logLik,
           label = sprintf("lambda* = %.2f", BC_lambda_max),
           size = 5, vjust = 0.9, hjust = 0.5, color = "red") +
  annotate("text",
           x = 10, y = BC_95ci,
           label = "95% cutoff",
           size = 5, vjust = 0.9, hjust = 1, color = "blue") +
  labs(
    x = "lambda",
    y = "Log-Likelihood",
...
After the Boxcox transformation, those coefficients are no longer in the same units, so we can not directly comparing their coefficients. Instead, we have compared their R^2 where it improved from	0.7579 to 0.8016. Standardized residuals versus fitted-value plots were also examined for both models. The Box–Cox transformation produced a residual pattern that was slightly improved where those residuals are more evenly scattered around zero compared with the untransformed model.

```{r boxcox transformation new model,  echo=FALSE, message=FALSE, warning=FALSE }


## Testing on transformed new model:
y <- main_data_for_modeling$time_milliseconds
BC_lambda_max <- BC_lambda_max

main_data_for_modeling_BC <- main_data_for_modeling %>%
  mutate(y_bc = (time_milliseconds^BC_lambda_max - 1) / BC_lambda_max)
...
model_v7 <- lm(y_bc ~
                ...
                event_ordinal  * stroke_count +
                event_ordinal  *I(stroke_count^2)  , 
              data = main_data_for_modeling_BC  )
...
model_summary_list<- rlang::set_names(
  list( model_final, model_v7),
  c("model_final", "model_v7")
)
...
```{r boxcox transformation residual compare,  echo=FALSE, message=FALSE, warning=FALSE , fig.width=10, fig.height=4}

#| echo: false
#| fig-cap: "Boxcox Transformed Model Residual Compare"
##| fig-width: 12
##| fig-height: 10
#| fig-pos: H
#| fig-cap-location: top
#| label: plot_boxcox_residual
#| message: false
#| warning: false
#| 
...
plot(fitted(model_final), rstandard(model_final),
     xlab = "Fitted values", ylab = "standardized residuals",
     main = "model_final",
     ylim = c(-6, 3))
abline(h = 0, lty = 2)
...
plot(fitted(model_v7), rstandard(model_v7),
     xlab = "Fitted values (Box–Cox scale)", ylab = "standardized residuals",
     main = "model_v7: (BC scale)",
     ylim = c(-6, 3))
abline(h = 0, lty = 2)
abline(h = c(-2, 2), lty = 3)
...
<!-- ## Sample size re-estimation (Pending) -->
