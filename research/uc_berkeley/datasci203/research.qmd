...
Compared to @eq-regression_model_v1 and @eq-regression_model_v2, we achieved a much larger $R^2$ (`r model_v3_summary_data$r_squared`), but incrementally larger adjusted $R^2$ (`r model_v3_summary_data$adjusted_r_squared`) and with estimators with higher $p$ values, indicating that this model suffers from overfitting.`