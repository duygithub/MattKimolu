...
We use @fig-exploratory_data_stroke_count_elapsed_time_plot to help inform us of how to model @eq-regression_model. We use the summary statistics, excluding those without standard deviations, in @tbl-summary_statistics_exploratory_data to generate test data from $N(\bar{x}, s)$.
...
main_data_y_min <- 25000
main_data_y_max <- 40000
...
* 50L2
* 100IM (we don't attempt to find the optimal $S_C$ for this value because we have a data set that is far too small for analysis.)
...
To test if stroke count significantly affects swimming time, we have compared 2 models by using the ANOVA test. The reduced model is model_v0 which will be used as baseline model, it only includes “data_entry_type” and “ordinal_events”. And the full model will be model_v4 which includes “stroke_count”, its quadratic term and other interaction terms with “data_entry_type” and “ordinal_events”.

The result of the F-test from ANOVA gave us a p-value of `r hypothesis_p_value2` which is significant, so we reject null hypothesis. This result indicates that stroke count is meaningful for predicting swim time. 

### Result 2: Optimal Stroke count per data type and ordinal events
...
We have attached table here for optimal stroke count for each data entry type and each ordinal event. Notice that when the ordinal event is lower than 6, the result indicates that the fewer the stroke count is, the lower the swim time is. This is because of we had interaction terms in the model, such as stroke count by ordinal event, and also stroke count square by ordinal event. 
...
Compared to @eq-regression_model_v1, we achieved a much larger $R^2$ (`r model_v2_summary_data$r_squared`), but incrementally larger adjusted $R^2$ (`r model_v2_summary_data$adjusted_r_squared`) and with estimators with higher $p$ values, indicating that this model suffers from overfitting.`
...
Compared to @eq-regression_model_v1 and @eq-regression_model_v2, we had a much lower $R^2$ (`r model_v3_summary_data$r_squared`), but the largest adjusted $R^2$ (`r model_v3_summary_data$adjusted_r_squared`) and statistical significance. However, this model doesn't make sense to us because the model is missing interaction terms, which should matter.


## Model V4: Added interaction term with event ordinal
...
\hat{T}_{Lap} = & \hat{\beta}_0 + \hat{\beta}_1 S_C + \hat{\beta}_2 {S_C}^2 \\
  & + \hat{\gamma}_{50L2} I_{50L2} \\
  & + \hat{\gamma}_{100IM} I_{100IM} \\
  & + \hat{\eta}_0 O + \hat{\eta}_0 O S_C + \hat{\eta}_0 O {S_C}^2
\end{split}
$$


We added variable event ordinal and also added interaction of stroke count with event ordinal to get to see if we can get to statistical significance.


```{r summary_regression_model_v4, echo=FALSE, results='asis'}
...
model_v4_summary_data <- calculate_model_coefficients_summary(model_v4)

knitr::kable(model_v4_summary_data$table_summary_data, digits = 3, caption = "Model v4 Summary")
```


{{< pagebreak >}}

## MCompare between Models
...
model_summary_list<- rlang::set_names(
  list(model_v1, model_v2, model_v3, model_v4),
  c("model_v1", "model_v2", "model_v3", "model_v4")
)
...
#| fig-pos: H
#| fig-cap-location: top
#| label: plot_model_v4_no_outliers
#| message: false
#| warning: false
