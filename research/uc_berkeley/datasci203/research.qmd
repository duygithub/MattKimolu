...
## Updated by Eric, 2025-12-09, replaced with another power analysis function:
...
```{r group_modeling}
# data_for_modeling_group_2550L1 <- prep_data_for_modeling_group_2550L1(main_data)
data_for_modeling_group_2550L1 <- main_data_for_modeling %>%
  filter(data_entry_type == '25 or 50L1')
model_subgroup_2550L1 <-lm(time_milliseconds ~ 
                stroke_count + 
                I(stroke_count^2),
                data_for_modeling_group_2550L1)

data_for_modeling_group_50L2 <- main_data_for_modeling %>%
  filter(data_entry_type == '50L2')
model_subgroup_50L2 <-lm(time_milliseconds ~ 
                stroke_count + 
                I(stroke_count^2) +
                I(stroke_count^3),
                data_for_modeling_group_50L2)

data_for_modeling_group_100IM <- main_data_for_modeling %>%
  filter(data_entry_type == '100IM')
model_subgroup_100IM <-lm(time_milliseconds ~ 
                stroke_count + 
                I(stroke_count^2),
                data_for_modeling_group_100IM)
...
We added variable event ordinal and also added interaction of storke count with event ordinal to get to see if we can get to statistical significance.
...
We would like to improve our Model v4, we have tried the Boxcox transformation. The result of the Boxcox transformation indicated that the when lambda = `r BC_lambda_max`, the likelihood reached max. It indicates that the Boxcox transformation suggests that to transfer the response variable with lambda of `r BC_lambda_max`. So our $y^{(\lambda)}$ after  transformation is the following:
