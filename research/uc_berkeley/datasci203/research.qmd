...
prepped_exploratory_data <- mutate(
  ...
  summary_stats2 <-prepped_exploratory_data %>%
  ...
  summary_exploratory_data_stats <- summary_stats2
  
knitr::kable(
 summary_exploratory_data_stats,
  caption = "Summary Statistics for Elapsed Time (in milliseconds) by Stroke Count",
...
exploratory_summary <- summary_exploratory_data_stats
power_analysis_simulation_count <- 1000
power_analysis_data_size <- 45
alpha <- 0.05
...
data_files_names <- c("20251118_-8.csv", "20251121_-8.csv", "20251122_-8.csv")
...
\hat{T}_{Lap} = & \hat{\beta}_0 + \hat{\beta}_1 S_C + \hat{\beta}_2 {S_C}^2 \\
 ...
 & + \hat{\eta} U + \hat{\theta}_0 O + \hat{\theta}_1 O^2 
\end{split}
...
``` {r descriptive analysis by data entry type}


summary_data_entry_type <-prepped_main_data2 %>%
    group_by(data_entry_type) %>%
    summarize(n=length((event_ordinal)),
...
kable(summary_data_entry_type , digits = 1, caption = "Descriptive summary by data entry types")
